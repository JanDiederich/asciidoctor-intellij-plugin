...
public class AsciiDocRunnerForJavaScript extends AsciiDocRunnerArbitrary {
   ...
   * <ol>
   *  <li>Global SDKs (Node.js interpreter configured in IntelliJ).
  ...
  public static String findJavaScriptInterpreter() {
    /*
     ...
     and may crash during runtime with a ClassNotFoundException. Even the class actually exists,
     but just in another version of the JetBrains JavaScript plugin.
    */
  ...
  public static List<AsciiDocSuggestedParameter> suggestedParameters() {
    return List.of(//
      new AsciiDocSuggestedParameter("--input-type=module",
        "Use top-level import and await statements. Incompatible with \"-p\".", "14.0"),
      new AsciiDocSuggestedParameter("--no-warnings", "No warnings.", null),
      new AsciiDocSuggestedParameter("-p",
        "Evaluate script and print result. Incompatible with \"--input-type=module\".", null)
      //
    );
  }
}

...
public class AsciiDocRunnerForTypeScript extends AsciiDocRunnerArbitrary {
  ...
  @Nullable List<String> codeRunParameters(AsciiDocScriptLanguageSetting languageSetting) {
    ...
    strings.add("tsx");
    if (languageSetting.getParameters() != null) {
      strings.addAll(languageSetting.getParameters());
    }
    // Under Windows, execute a temporary file, direct string execution is problematic.
    if (!SystemInfo.isWindows) {
      strings.add("-e");
    }
    return strings;
  ...
  public static List<AsciiDocSuggestedParameter> suggestedParameters() {
    ...
    if (!SystemInfo.isWindows) {
      parameters.add(
        new AsciiDocSuggestedParameter("--input-type=module", "Recommended: Use top-level import and await statements.",
          "14.0"));
    }
  ...
  /**
   * <ul>
   * <li><code>NotNull</code> under Windows
   * <li><code>Null</code> under Linux
   * </ul>
   * Windows (double-) quotes handling is so bad under Windows, that even with the
   * best script file (<code>npx.ps1</code>, not <code>npx.cmd</code>),
   * it's still too error-prone.
   * The whole code <em>must</em> be written to a temporary file and the file be executed.
  ...
  TempFileInfo getTempFileInfo() {
    return SystemInfo.isWindows ? new TempFileInfo(".ts") : null;
  }
